...
class AttendanceScraper:
    def __init__(self, use_mock=False):
        ...
        self.debug_root = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "scrape"
        )
    ...
    def _snapshot_page(self, page, debug_dir, prefix):
        ...
        try:
            page.screenshot(
                path=os.path.join(debug_dir, f"{prefix}_{ts}.png"), full_page=True
            )
        except:
            pass
        try:
    ...
    def _snapshot_frames(self, page, debug_dir, prefix):
        ...
        for idx, frame in enumerate(page.frames):
            ...
            safe_name = "".join(
                c if c.isalnum() or c in ("_", "-") else "_" for c in frame_name
            )
            try:
                html = frame.content()
                self._write_debug_text(
                    debug_dir, f"{prefix}_{ts}_{idx}_{safe_name}.html", html
                )
    ...
    def _trigger_captcha_refresh(self, frame):
        ...
        if not did_trigger:
            try:
                refresh_icon = frame.locator(
                    "img[title*='Refresh'], img[src*='refresh']"
                )
    ...
    def _refresh_captcha_and_get_base64(
        self, page, frame=None, debug_dir=None, prefix=None
    ):
        ...
        if login_frame:
            ...
            if debug_dir and prefix:
                self._write_login_form_state(
                    debug_dir, f"{prefix}_form_state.json", login_frame
                )
    ...
    def refresh_captcha(self, session_id):
        ...
        if not captcha_base64:
            ...
            return {
                "success": False,
                "message": "Could not refresh CAPTCHA from portal.",
            }
    ...
    def submit_captcha_and_scrape(self, session_id, captcha_text=None, auto_ocr=False):
        ...
        self._write_debug_json(
            debug_dir,
            f"attempt_{attempt_no}_input.json",
            {
        ...
        try:
            ...
            if not login_frame:
                raise Exception(
                    "Could not find CAPTCHA field. Session may have expired."
                )
            ...
            if auto_ocr and not captcha_text:
                    ...
                    try:
                        ...
                        if ocr_attempt > 0:
                            new_captcha_b64 = self._refresh_captcha_and_get_base64(
                                ...
                                frame=login_frame,
                                debug_dir=debug_dir,
                                prefix=f"ocr_retry_{ocr_attempt}",
                            )
                            time.sleep(1)
                            login_frame = self._find_login_frame(page)
                            if not login_frame:
                            ...
                            try:
                                ...
                                if captcha_el.count() > 0:
                                    captcha_bytes_for_ocr = captcha_el.screenshot()
                            except:
                                pass
                        ...
                        if captcha_text:
                            ...
                            self._write_debug_text(
                                debug_dir,
                                f"attempt_{attempt_no}_ocr_empty_{ocr_attempt}.txt",
                                "empty",
                            )
                    except Exception as e:
                        self._write_debug_text(
                            debug_dir,
                            f"attempt_{attempt_no}_ocr_error_{ocr_attempt}.txt",
                            str(e),
                        )
                ...
                if not captcha_text:
                    ...
                    return {
                        "success": False,
                        "message": "OCR failed to read CAPTCHA after multiple attempts. Please enter manually.",
                        "retryable": True,
            ...
            if login_state == "authenticated":
                ...
                self._snapshot_page(
                    page,
                    debug_dir,
                    f"05_login_screen_still_visible_attempt_{attempt_no}",
                )
                return {
                    "success": False,
                ...
                self._snapshot_page(
                    page, debug_dir, f"05_login_pending_attempt_{attempt_no}"
                )
                return {
                    "success": False,
            ...
            return {
                ...
                "captcha_base64": self._get_fresh_captcha_from_page(page)
                or (
                    self._get_fresh_captcha_base64(login_frame)
                    if "login_frame" in locals() and login_frame
                    else None
                ),
            }
        ...
        for attempt in range(max_attempts):
            try:
                for frame in page.frames:
                    try:
                        if all(
                            frame.locator(selector).count() > 0
            ...
            try:
                for frame in page.frames:
                    try:
                        ...
                        has_semester = (
                            frame.locator("select[name='semester']").count() > 0
                        )
                        has_submit = (
                            frame.locator(
                        ...
                        if (
                            has_submit
                            and (has_year or has_semester)
                            and not is_activity_tree
                        ):
                            return frame
                    except:
                        pass
    ...
    def _load_attendance_records(
        self, page, content_frame, debug_dir, attempt_no, rollno
    ):
        ...
        for year in candidates["years"]:
            for semester in candidates["semesters"]:
                ...
                if attendance_data:
                    attendance_payload = (
                        getattr(self, "_last_attendance_payload", {}) or {}
                    )
                    selected_year_value = year or attendance_payload.get(
                        "student", {}
                    ).get("academic_year", "")
                    selected_semester_value = semester or attendance_payload.get(
                        "student", {}
                    ).get("semester", "")
                    ...
                    merged_payload["status_legend"].update(
                        attendance_payload.get("status_legend") or {}
                    )
                    merged_payload["calendar"].extend(
                        attendance_payload.get("calendar") or []
                    )
                    ...
                    self._write_debug_text(
                        debug_dir,
                        f"09_attendance_form_html_attempt_{attempt_no}.html",
                        last_html,
                    )
                    self._write_debug_json(
                        debug_dir,
        ...
        if all_attendance_data:
            synced_years = self._ordered_unique(
                [item.get("year") for item in success_filters]
            )
            synced_semesters = self._ordered_unique(
                [item.get("semester") for item in success_filters]
            )
            merged_payload["selected_year"] = (
                synced_years[0] if len(synced_years) == 1 else "Multiple"
            )
            merged_payload["selected_semester"] = (
                synced_semesters[0] if len(synced_semesters) == 1 else "All synced"
            )
            ...
            for frame in page.frames:
                try:
                    ...
                    if "logout" in text and "welcome" in text:
                        return True
                    if (
                        "personal info" in text
                        and "attendance" in text
                        and "plum erp" in text
                    ):
                        return True
                except:
                    pass
    ...
    def _find_attendance_link(self, page, max_attempts=20):
        return self._find_portal_link_by_text(
            page, "My Attendance", max_attempts=max_attempts
        )
            ...
            try:
                for frame in page.frames:
                    try:
                        ...
                        if (
                            link_text.lower() not in visible_text
                            and expected not in "".join(visible_text.split())
                        ):
                            continue
    ...
    def _ensure_activity_menu_loaded(self, page, debug_dir, attempt_no, force=False):
        ...
        activity_link = self._find_portal_link_by_text(
            page, "My Activities", max_attempts=1
        )
        if not activity_link:
            return False

        self._write_debug_json(
            debug_dir, f"06_my_activities_link_attempt_{attempt_no}.json", activity_link
        )
        try:
            self._open_portal_link(page, activity_link)
            self._snapshot_page(
                page, debug_dir, f"06_after_my_activities_nav_attempt_{attempt_no}"
            )
            self._snapshot_frames(
                page, debug_dir, f"06_after_my_activities_nav_attempt_{attempt_no}"
            )
            ...
            self._write_debug_text(
                debug_dir,
                f"06_my_activities_nav_error_attempt_{attempt_no}.txt",
                str(e),
            )
            return False
    ...
    def _extract_portal_catalog(self, page):
            ...
            for frame in page.frames:
                ...
                for node in soup.find_all(["b", "a"]):
                    ...
                    if not text:
                        image = node.find("img")
                        text = (
                            (image.get("title") or image.get("alt") or "").strip()
                            if image
                            else ""
                        )
                    if not text:
        ...
        for surface, markers in important.items():
            if any(
                any(marker in link["text"].lower() for marker in markers)
    ...
    def _open_portal_link(self, page, link_info):
        ...
        for frame in candidate_frames:
            ...
            try:
                ...
                if last_click_result and last_click_result.get("clicked"):
                    ...
                    if target_frame:
                        try:
                            target_frame.wait_for_load_state(
        ...
        if "plum_url.php" in href:
            reason = (
                (last_click_result or {}).get("reason")
                or (last_click_result or {}).get("error")
                or "anchor click failed"
            )
    ...
    def _preprocess_captcha_image(self, image):
        ...
        try:
            ...
            # Aggressive upscale for tiny captcha images
            w, h = image.size
            if w < 150:
            ...
            # Binary threshold - key for separating captcha chars
            img = img.point(lambda x: 255 if x > 100 else 0, "1")
    ...
    def _google_vision_solve_captcha(self, image_bytes):
        """Solve captcha using Google Cloud Vision API (free 1000/month).
    ...
    def _ocr_captcha_from_page(self, page, frame):
        """Attempt to read CAPTCHA via OCR with preprocessing and retries."""
        try:
            import pytesseract
            ...
            try:
                captcha_img_element = frame.locator(
            ...
            # Fallback to original image if processed didn't work
            if not best_result:
                for psm in [8, 7]:
                    try:
                        config = f"--oem 3 --psm {psm} -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
    ...
    def _normalize_attendance_date(self, day_text, academic_year):
        ...
        try:
            parsed = datetime.strptime(
                f"{year}-{month_abbr.title()}-{int(day):02d}", "%Y-%b-%d"
            )
    ...
    def _parse_special_notes(self, soup):
        ...
        for match in note_pattern.finditer(soup.get_text("\n", strip=True)):
            ...
            try:
                parsed_date = datetime.strptime(date_text, "%d-%m-%Y").strftime(
                    "%Y-%m-%d"
                )
            except:
    ...
    def _parse_attendance_payload(self, html_content):
        ...
        def selected_value(selector):
            ...
            return (
                (selected.get("value") or selected.get_text(" ", strip=True)).strip()
                if selected
                else None
            )
        ...
        semester = selected_value("select[name='sem']") or selected_value(
        ...
        if not any(
            "Total Classes" in table.get_text(" ", strip=True) for table in tables
        ):
            self._last_attendance_payload = payload
            return payload

        header_pattern = re.compile(
            r"Name:\s*(.*?)(?:\s*\(([^)]+)\))?\s*,\s*Semester\s*:\s*(\d+)", re.I
        )
        ...
        for line in soup.get_text("\n", strip=True).splitlines():
            ...
            if legend_match and legend_match.group(1) not in subject_codes:
                payload["status_legend"][legend_match.group(1)] = legend_match.group(
                    2
                ).strip()
                last_subject_code = None
                continue
        ...
        subject_count = min(
            len(subject_codes), len(total_values or []), len(present_values or [])
        )
        for index in range(subject_count):
            ...
            total_absent = (
                to_int(absent_values[index])
                if absent_values and index < len(absent_values)
                else max(total_classes - total_present, 0)
            )
            ...
            absent_dates = [
                event["date"] for event in day_wise if event.get("absent_count", 0) > 0
            ]
            special_events = special_notes.get(code, [])
            for event in day_wise:
                for special_code in event.get("special_codes", []):
                    if any(
                        note["date"] == event["date"] and note["mark"] == special_code
                        for note in special_events
                    ):
                        continue
                    special_events.append(
                        {
                            "date": event["date"],
                            "mark": special_code,
                            "description": payload["status_legend"].get(
            ...
            payload["subjects"].append(
                {
                    ...
                    "special_events": sorted(
                        special_events, key=lambda item: item.get("date", "")
                    ),
                }
            )
    ...
    def _parse_day_wise_html(self, html_content):
        ...
        for table in tables:
            if (
                "Date" in table.text
                or "Status" in table.text
                or "Absent" in table.text
                or "Present" in table.text
            ):
                target_table = table
                break
    ...
    def _compute_full_analysis(
            ...
            attended_days = len(
                {
                    event.get("date")
                    for event in day_wise
                    if event.get("present_count", 0) > 0
                }
            )
            absent_days = len(
                {
        ...
        lowest_subject = min(
            subjects, key=lambda item: item.get("percentage", 1000), default=None
        )
        strongest_subject = max(
            subjects, key=lambda item: item.get("percentage", -1), default=None
        )
        total_skippable_75 = sum(
            max(subject.get("skippable_75", 0), 0) for subject in subjects
        )
        all_absences = []
        all_specials = []
        for subject in subjects:
        ...
        return {
            ...
            "insights": {
                ...
                "recent_absences": sorted(
                    all_absences, key=lambda item: item.get("date") or "", reverse=True
                )[:12],
                "special_events": sorted(
            ...
            "source": {
                "academic_year": attendance_payload.get("selected_year")
                or attendance_payload.get("student", {}).get("academic_year", ""),
                "semester": attendance_payload.get("selected_semester")
                or attendance_payload.get("student", {}).get("semester", ""),
                "available_years": attendance_payload.get("available_years", []),
                "available_semesters": attendance_payload.get(
                    "available_semesters", []
                ),
    ...
    def get_leave_prediction(self, subject_data, threshold=75.0):
        ...
        if total == 0:
            return {
                "status": "safe",
                "message": f"No classes have been held yet.",
                "skippable_classes": 0,
            }
        ...
        if percentage >= threshold:
            ...
            needed = math.ceil(
                (threshold_decimal * total - attended) / (1 - threshold_decimal)
            )
            return {
                "status": "danger",
